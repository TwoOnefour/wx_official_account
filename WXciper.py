...
class XMLParse(object):
    ...
    def get_message(self, xmltext):
        xml_tree = ET.fromstring(xmltext)
        event = xml_tree.find("event")
        msg = xml_tree.find("content").find("msg")
        ...
        return [xml_tree.find("userid").text, msg, xml_tree.find("from").text, event, xml_tree.find("appid").text]
...
class WXBizMsgCrypt(object):
    ...
    def EncryptMsg(self, sReplyMsg, sNonce, timestamp=None):
        ...
        return ret, encrypt
    def DecryptMsg(self, sPostData):
        # 检验消息的真实性，并且获取解密后的明文 , sMsgSignature, sTimeStamp, sNonce
        ...
        pc = Prpcrypt(self.key)
        ret, xml_content = pc.decrypt(sPostData, self.appid)
